...
namespace demos
{
    ...
    void D01HelloQuad::Draw() const
    {
        ...
        {
            mouseNdcX = (window::input::MouseWindowX / static_cast<float>(window::WindowWidth)) * 2.0f - 1.0f;
            mouseNdcY = 1.0f - (window::input::MouseWindowY / static_cast<float>(window::WindowHeight)) * 2.0f;
        }
        shader.SendUniform("uMouse", glm::vec2(mouseNdcX, mouseNdcY));
    ...
    {
        ImGui::Text("Hello Quad - Ginam Park");
        ImGui::Text("Time: %.2f", static_cast<double>(window::ElapsedTime));
